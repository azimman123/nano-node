...
TEST (network, duplicate_detection)
{
	...
	ASSERT_TIMELY (5s, node1.stats.count (nano::stat::type::filter, nano::stat::detail::duplicate_publish) == 0);
	tcp_channel->send (publish);
	ASSERT_TIMELY (5s, node1.stats.count (nano::stat::type::filter, nano::stat::detail::duplicate_publish) == 1);
}
